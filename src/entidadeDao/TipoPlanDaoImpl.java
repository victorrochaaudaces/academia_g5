...
public class TipoPlanDaoImpl implements TipoPlanDao {
    ...
    @Override
    public List<TipoPlan> pesquisarTodo() throws Exception {
        String comando = "SELECT * FROM tipo_plan";
        List<TipoPlan> planos = new ArrayList<>();
        try {
            conn = ConnectionDb.ConDb();
            prepareSql = conn.prepareStatement(comando);
            resultado = prepareSql.executeQuery();
            ...
            System.out.println("Erro ao pegar tipos de planos " + e.getMessage());
        } finally {
            conn.close();
            prepareSql.close();
            resultado.close();
